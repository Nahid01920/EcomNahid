...
const CartProducts = () => {
  const { cart } = useSelector((state: StateType) => state?.nahid);
  return (
    <div>
      {cart?.length > 0 ? (
        <>
        ...
        <div className="bg-white h-96 my-10 flex flex-col gap-4 items-center justify-center py-5 rounded-lg border border-gray-200 drop-shadow-2xl">
          ...
          <p className="text-base max-w-[700px] text-center text-gray-600 tracking-wide leading-6">
            ...
            nisi hic consequatur necessitatibus laborum ut neque repellat
            blanditiis veritatis ex.
          </p>
          <Button
            href="/"
            className="bg-skyColor/90 text-gray-100 px-8 py-4
            rounded-md hover:bg-skyColor duration-200 uppercase 

...
import { addToFavorite } from "@/redux/nahidSlice";
import toast from "react-hot-toast";

const Sidebar = ({ product }: { product: ProductType }) => {
  const { favorite } = useSelector((state: StateType) => state?.nahid);
  ...
  const handleFavorite = () => {
    ...
    console.log("nahid hasan nahid");
  };

  return (
    <div
      className="absolute right-2
     bottom-44 border flex flex-col text-2xl border-borderColor 
     bg-white rounded-md overflow-hidden transform translate-x-20
     group-hover:translate-x-0 duration-300 z-40"
    >
